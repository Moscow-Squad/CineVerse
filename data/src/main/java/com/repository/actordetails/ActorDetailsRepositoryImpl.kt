...
class ActorDetailsRepositoryImpl(
    ...
    override suspend fun getActorDetails(actorId: Int): ActorDetails  = tryToExecute(exception = NoMoviesFoundException){
        actorDetailsRemoteDataSource.getActorDetails(actorId).toDomain()
    }

    override suspend fun getGallery(actorId: Int): List<String> = tryToExecute(exception = NoMoviesFoundException){
        actorDetailsRemoteDataSource.getGallery(actorId).map { it.toDomain() }
    }

    override suspend fun getBestOfMovies(actorId: Int): List<Movie> = tryToExecute(exception = NoMoviesFoundException){
